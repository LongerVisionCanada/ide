...
async function openAction() {
  if (useLvt()) {
    gLvtFile = await puter.ui.showOpenFilePicker();
    openFile(await (await gLvtFile.read()).text(), gLvtFile.name);
  } else {
...
async function saveAction() {
  if (useLvt()) {
    if (gLvtFile) {
      gLvtFile.write(sourceEditor.getValue());
    } else {
      gLvtFile = await puter.ui.showSaveFilePicker(
        sourceEditor.getValue(),
        getSourceCodeName()
      );
      setSourceCodeName(gLvtFile.name);
    }
  } else {
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  $selectLanguage.change(function (event, data) {
    let skipSetDefaultSourceCodeName =
      (data && data.skipSetDefaultSourceCodeName) || !!gLvtFile;
